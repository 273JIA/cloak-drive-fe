                ...
                <div class="upload-content" id="upload-content">
                    <div class="drag-content">
                        ...
                        <div class="drag-text-content">
                            <el-link :underline="false" type="info">Drag the file here, or </el-link>
                            <el-link :underline="false" type="primary">Click to upload</el-link>
                        </div>
                    </div>
                </div>
            </el-dialog>
...
const rebindUploader = () => {
    if (uploader && !uploader.support) {
        ElMessage.error('This browser does not support simple-uploader, please change your browser and try again')
    }
    ...
    try {
        files.forEach((f) => {
            ...
            if (f.size > panUtil.getMaxFileSize()) {
                throw new Error('File:' + f.name + 'size exceeds the maximum upload file limit（' + panUtil.translateFileSize(panUtil.getMaxFileSize()) + '）')
            }
            let taskItem = {
                target: f,
                filename: f.name,
            ...
            MD5(f.file, (e, md5) => {
                ...
                fileService.secUpload({
                    ...
                    if (res.code === 0) {
                        ElMessage.success('File:' + f.name + ' upload completed')
                        f.cancel()
...
const doMerge = (file) => {
    ...
    fileService.merge({
        ...
        ElMessage.success('File:' + file.name + ' upload completed')
