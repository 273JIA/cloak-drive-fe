            ...
            <div>
                <div v-if="step === 1">
                    <el-form label-width="100px" :rules="shareFileRules" ref="shareFormRef"
                        ...
                        <el-form-item label="ShareName" prop="shareName">
                            <el-input type="text"
                                      ref="shareNameEl"
                        ...
                        <el-form-item label="Share Code">
                            <el-radio-group v-model="shareFileForm.shareType">
                                <el-radio disabled label="0">Share Code</el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item label="Valid For">
                            <el-select v-model="shareFileForm.shareDayType">
                                <el-option label="permanently valid" value="0"></el-option>
                ...
                <div v-if="step === 2">
                    <el-form label-width="100px"
                             status-icon
                             @submit.native.prevent>
                        <el-form-item label="Share Url" prop="shareUrl">
...
const shareFileRules = reactive({
    shareName: [
        {required: true, message: 'Please enter the name of your share', trigger: 'blur'}
    ]
})
...
const shareFile = () => {
    if (!props.item && (!multipleSelection.value || multipleSelection.value.length === 0)) {
        ElMessage.error('Please select the files to share')
        return
    }
    if (props.item) {
        shareTitle.value = 'Share file（' + handleFilename(props.item.filename) + ')'
    } else {
        if (multipleSelection.value.length === 1) {
            shareTitle.value = 'Share file（' + handleFilename(multipleSelection.value[0].filename) + ')'
        } else {
            shareTitle.value = 'Share file（' + handleFilename(multipleSelection.value[0].filename) + '..)'
        }
    }
    ...
    await shareFormRef.value.validate((valid, fields) => {
        if (valid) {
            ...
            shareService.createShare({
                ...
                loading.value = false
                shareTitle.value = 'Congratulations! Sharing was successful!'
...
const copy = async () => {
    try {
        let shareMessage = 'Url:' + shareResultForm.shareUrl + '\nShare Code:' + shareResultForm.shareCode + '\nShare it with your friends!'
