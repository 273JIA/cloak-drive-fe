...
const renameRules = reactive({
    filename: [
        {required: true, message: 'Please enter a new file name', trigger: 'blur'}
    ]
})
...
const renameFile = () => {
    ...
    if (!multipleSelection.value || multipleSelection.value.length == 0) {
        ElMessage.error('Please select the file to rename')
        return
    }
    if (multipleSelection.value.length > 1) {
        ElMessage.error('Please select a file to rename')
        return
    }
