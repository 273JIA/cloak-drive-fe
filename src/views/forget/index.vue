<template>
    <div class="content">
        ...
        <div class="form" v-if="currentStep === 1">
            ...
            <el-button type="warning" class="submit" :loading="false" @click="goStepTwo" round>Next</el-button>
            <div class="login-link-content">
                <el-link type="primary" @click="goLogin">Login</el-link>
            </div>
        </div>
        <div class="form" v-if="currentStep === 2">
            <h2>Verify Password Answer</h2>
            <div class="forget-form-content">
                <div class="forget-form-item">
                    <label>
                        <span>Your security question is</span>
                        <div class="question-content">{{ question }}</div>
                    </label>
                    <label>
                        <span>Your password answer is</span>
                        <input type="text" v-model="answer" @keyup.enter="goStepThree" ref="answerEl"/>
                    </label>
                </div>
            </div>
            <el-button type="warning" class="submit" :loading="loading" @click="goStepThree" round>Next</el-button>
            <div class="login-link-content">
                <el-link type="primary" @click="goLogin">Login</el-link>
            </div>
        </div>
        <div class="form" v-if="currentStep === 3">
            ...
            <el-button type="warning" class="submit" :loading="false" @click="resetPassword" round>Submit</el-button>
            <div class="login-link-content">
                <el-link type="primary" @click="goLogin">Login</el-link>
            </div>
        </div>
...
const goStepTwo = () => {
    if (!username.value) {
        ElMessage.error('Please enter the username to change your password')
        return
    }
    loading.value = true
...
const resetPassword = () => {
    if (!token.value) {
        ElMessage.error('Due to prolonged inactivity, you will be returned to Step One to reapply for password reset')
        goStepOne()
        return
    }
    if (checkPassword()) {
        ...
        userService.resetPassword({
            ...
            ElMessage.success('Password reset successfully! You are about to be redirected to the login page')
