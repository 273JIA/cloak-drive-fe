            ...
            <div class="pan-share-list-wrapper">
                <el-card shadow="always" class="pan-share-list-card">
                        ...
                        <div class="pan-share-list-card-header-share-info-content">
                            ...
                            <div class="pan-share-list-card-header-time">
                                <span class="pan-share-list-card-header-create-date">
                                    <i class="fa fa-clock-o"/>Share Date:{{ shareDate }}
                                </span>
                                <span class="pan-share-list-card-header-expire-date">
                                     <i class="fa fa-clock-o"/>Expire Date:{{ shareExpireDate }}</span>
                            </div>
                        </div>
                        <div class="pan-share-list-card-button-group">
                            <el-button type="success" size="default" round @click="saveFiles(undefined)">Save to my drive
                                <el-icon
                                    class="el-icon--right">
                                    <DocumentCopy/>
                                </el-icon>
                            </el-button>
                            <el-button type="info" size="default" round @click="downloadFile">Download
                                <el-icon
                                    class="el-icon--right">
                                    <Download/>
                                </el-icon>
                    ...
                    <div v-if="shareCancelFlag" class="pan-share-list-card-error-message">
                        <span>Sorry this share has expired or is no longer valid</span>
                    </div>
                            ...
                            <el-table-column
                                ...
                                <template #default="scope">
                                    ...
                                    <div class="file-operation-content">
                                        <el-tooltip class="item" effect="light" content="Save to my drive" placement="top">
                                            <el-button type="success" icon="DocumentCopy" size="small" circle
                                                       @click="saveFiles(scope.row)"/>
                                        </el-tooltip>
                                        <el-tooltip class="item" effect="light" content="Download" placement="top">
        ...
        <el-dialog
            ...
            <template #footer>
                <span class="dialog-footer">
                    ...
                    <el-button type="primary" @click="doLogin" :loading="loading">Confirm</el-button>
                </span>
            </template>
        </el-dialog>
        <el-dialog
                ...
                <div class="pan-share-code-wrapper">
                    <el-card :header="shareCodeHeader" shadow="always" class="pan-share-code-card">
                        <el-form
                            ...
                            :model="shareCodeForm"
                            @submit.native.prevent>
                            <el-form-item label="Share Code" prop="shareCode">
                                <el-input type="text"
                                          ref="shareCodeEl"
                            ...
                            <el-form-item>
                                <el-button type="primary" :loading="loading" @click="doCheckShareCode">Confirm</el-button>
                            </el-form-item>
                        </el-form>
                    </el-card>
                </div>
...
const checkUsername = (rule, value, callback) => {
        if (!panUtil.checkUsername(value)) {
            callback('Please enter a 6-16 digit username containing only numbers and letters')
            return
        }
        callback()
    },
    checkPassword = (rule, value, callback) => {
        if (!panUtil.checkPassword(value)) {
            callback('Please enter a password of 8-16 digits')
            return
        }
        callback()
    }
...
const shareCodeFormRules = reactive({
    shareCode: [
        {required: true, message: 'Please enter the share code', trigger: 'blur'}
    ]
})
...
const refreshShareInfo = (data) => {
    ...
    shareCodeHeader.value = username + 'shares' + shareName
    shareDate.value = data.createTime
    if (data.shareDay === 0) {
        shareExpireDate.value = 'permanently valid'
    } else {
...
const openShareCodePage = () => {
    shareService.getSimpleShareDetail({
        ...
        if (res.code === 0) {
            ...
            shareCodeHeader.value = res.data.shareUserInfoVO.username + 'Share: ' + res.data.shareName
        } else {
...
const saveFiles = (newItem) => {
    if (newItem) {
        ...
        ElMessage.error('Please select a file to save')
        return
    }
    userService.infoWithoutPageJump(res => {
        if (res.code === 0) {
